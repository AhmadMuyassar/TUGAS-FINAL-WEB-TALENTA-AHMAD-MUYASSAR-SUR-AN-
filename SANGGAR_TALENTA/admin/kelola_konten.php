...
// ps() dan ps_img() sudah ada di includes/fungsi.php
...
<div class="container">
    ...
    <p class="alert-err">âš  <?php echo htmlspecialchars($_GET['error']); ?></p>
    <?php endif; ?>
        ...
        <div class="konten-form-col">
            <div class="form-card">
                ...
                <form method="POST" action="kelola_konten.php?tab=profil&aksi=simpan" enctype="multipart/form-data">
                    <input type="hidden" name="tentang_gambar_lama" value="<?php echo htmlspecialchars(ps_img($ps,'tentang_gambar')); ?>">

                    <div class="form-section-title">Tentang Sanggar</div>
                    <label>Judul</label>
                    <input type="text" name="tentang_judul" value="<?php echo htmlspecialchars(ps($ps,'tentang_judul')); ?>" required>
                    <label>Deskripsi Paragraf 1</label>
                    <textarea name="tentang_deskripsi" rows="4"><?php echo htmlspecialchars(ps($ps,'tentang_deskripsi')); ?></textarea>
                    <label>Deskripsi Paragraf 2</label>
                    <textarea name="tentang_deskripsi2" rows="3"><?php echo htmlspecialchars(ps($ps,'tentang_deskripsi2')); ?></textarea>
                    <label>Foto / Ilustrasi Sanggar (opsional)</label>
                    <input type="file" name="tentang_gambar" accept="image/*">
                    <?php $tg = ps_img($ps,'tentang_gambar'); if ($tg): ?>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                        <div><label>Angka 1</label><input type="text" name="stat1_angka" value="<?php echo htmlspecialchars(ps($ps,'stat1_angka','10+')); ?>"></div>
                        <div><label>Label 1</label><input type="text" name="stat1_label" value="<?php echo htmlspecialchars(ps($ps,'stat1_label','Tahun Berdiri')); ?>"></div>
                        <div><label>Angka 2</label><input type="text" name="stat2_angka" value="<?php echo htmlspecialchars(ps($ps,'stat2_angka','500+')); ?>"></div>
                        <div><label>Label 2</label><input type="text" name="stat2_label" value="<?php echo htmlspecialchars(ps($ps,'stat2_label','Pementasan')); ?>"></div>
                        <div><label>Angka 3</label><input type="text" name="stat3_angka" value="<?php echo htmlspecialchars(ps($ps,'stat3_angka','200+')); ?>"></div>
                        <div><label>Label 3</label><input type="text" name="stat3_label" value="<?php echo htmlspecialchars(ps($ps,'stat3_label','Koleksi Kostum')); ?>"></div>
                    </div>

                    <div class="form-section-title">Banner CTA</div>
                    <label>Judul CTA</label>
                    <input type="text" name="cta_judul" value="<?php echo htmlspecialchars(ps($ps,'cta_judul','Temukan Kostum Impian Anda')); ?>">
                    <label>Deskripsi CTA</label>
                    <textarea name="cta_deskripsi" rows="3"><?php echo htmlspecialchars(ps($ps,'cta_deskripsi')); ?></textarea>
                ...
                <form method="POST" action="kelola_konten.php?tab=slide&aksi=simpan" enctype="multipart/form-data">
                    ...
                    <?php endif; ?>
                    <p style="font-size:0.72rem;color:#999;margin-top:4px;">JPG/PNG, maks 5MB. Ukuran ideal: 1280Ã—600px.</p>
                ...
                <form method="POST" action="kelola_konten.php?tab=<?php echo $tab; ?>&aksi=simpan" enctype="multipart/form-data">
                    ...
                    <input type="text" name="jadwal" value="<?php echo htmlspecialchars($dataEdit['jadwal'] ?? ''); ?>"
                           placeholder="Contoh: Setiap Sabtu, 15.00â€“17.00">
        ...
        <div class="konten-list-col">
            ...
            <div class="form-card" style="background:#fffcf5;">
                ...
                <p style="font-size:0.88rem;font-weight:600;color:#333;margin-bottom:4px;">
                    <?php echo htmlspecialchars(ps($ps,'tentang_judul','(belum diisi)')); ?>
                </p>
                <p style="font-size:0.82rem;color:#555;line-height:1.6;">
                    <?php echo nl2br(htmlspecialchars(mb_substr(ps($ps,'tentang_deskripsi'),0,200))); ?>...
                </p>
                <?php $tg = ps_img($ps,'tentang_gambar'); if ($tg): ?>
                ...
                <p style="font-size:0.88rem;font-weight:600;color:#333;margin-bottom:4px;">
                    <?php echo htmlspecialchars(ps($ps,'cta_judul','(belum diisi)')); ?>
                </p>
                <p style="font-size:0.82rem;color:#555;">
                    <?php echo htmlspecialchars(mb_substr(ps($ps,'cta_deskripsi'),0,120)); ?>
                </p>
            ...
            <div class="slide-list-item">
                ...
                <div class="slide-list-info">
                    ...
                    <div class="slide-list-meta">
                        ...
                        <?php if (!empty($d['link_wa'])): ?>&nbsp;·&nbsp;ðŸŸ¢ WA<?php endif; ?>
                        <?php if (!empty($d['link_ig'])): ?>&nbsp;·&nbsp;ðŸ“¸ IG<?php endif; ?>
                        <?php if (!empty($d['link_tiktok'])): ?>&nbsp;·&nbsp;ðŸŽµ TikTok<?php endif; ?>
                    </div>
                </div>
            ...
            <div class="slide-list-item <?php echo $d['aktif'] ? '' : 'slide-nonaktif'; ?>">
                <div class="slide-thumb">
                    ...
                    <?php else: ?>
                    <div class="slide-thumb-empty">ðŸ–¼</div>
                    <?php endif; ?>
                </div>
                <div class="slide-list-info">
